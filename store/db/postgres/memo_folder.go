...
func (d *DB) ListMemoFolders(ctx context.Context, find *store.FindMemoFolder) ([]*store.MemoFolder, error) {
	...
	if len(find.UIDList) > 0 {
		...
		where = append(where, "uid IN ("+strings.Join(holders, ", ")+")")
	}
	...
	query := `SELECT id, uid, creator_id, created_ts, updated_ts, title, pinned
		FROM memo_folder
...
func (d *DB) UpdateMemoFolder(ctx context.Context, update *store.UpdateMemoFolder) error {
	...
	if v := update.Title; v != nil {
		set = append(set, "title = "+placeholder(len(args)+1))
		args = append(args, *v)
	}
	if v := update.Pinned; v != nil {
		set = append(set, "pinned = "+placeholder(len(args)+1))
		args = append(args, *v)
	}
	if v := update.UpdatedTs; v != nil {
		set = append(set, "updated_ts = "+placeholder(len(args)+1))
		args = append(args, *v)
	}
	if len(set) == 0 {
		return nil
	...
	stmt := "UPDATE memo_folder SET " + strings.Join(set, ", ") + " WHERE id = " + placeholder(len(args))
...
func (d *DB) DeleteMemoFolder(ctx context.Context, delete *store.DeleteMemoFolder) error {
	...
	if _, err := tx.ExecContext(ctx, "UPDATE memo SET folder_id = 0 WHERE folder_id = "+placeholder(1), delete.ID); err != nil {
		return errors.Wrap(err, "failed to detach memos from folder")
	}
	result, err := tx.ExecContext(ctx, "DELETE FROM memo_folder WHERE id = "+placeholder(1), delete.ID)
	if err != nil {
