...
func (d *DB) CreateMemoFolder(ctx context.Context, create *store.MemoFolder) (*store.MemoFolder, error) {
	stmt := `
		...
		VALUES (?, ?, ?, ?)
	`
	result, err := d.db.ExecContext(ctx, stmt, create.UID, create.CreatorID, create.Title, create.Pinned)
	if err != nil {
		return nil, err
	}
	...
	create.ID = int32(id)
	created, err := d.GetMemoFolder(ctx, &store.FindMemoFolder{ID: &create.ID})
	if err != nil {
		return nil, err
	}
	if created == nil {
		return nil, errors.New("failed to load created memo folder")
	}
	return created, nil
}

func (d *DB) GetMemoFolder(ctx context.Context, find *store.FindMemoFolder) (*store.MemoFolder, error) {
	list, err := d.ListMemoFolders(ctx, find)
	if err != nil {
		return nil, err
	}
	if len(list) == 0 {
...
func (d *DB) ListMemoFolders(ctx context.Context, find *store.FindMemoFolder) ([]*store.MemoFolder, error) {
	...
	query := "SELECT `id`, `uid`, `creator_id`, `created_ts`, `updated_ts`, `title`, `pinned` FROM `memo_folder` " +
		"WHERE " + strings.Join(where, " AND ") + " ORDER BY `pinned` DESC, LOWER(`title`) ASC, `id` ASC"
