...
export type Memo = Message<"memos.api.v1.Memo"> & {
  ...
  /**
   * Optional. The folder of the memo.
   * Format: users/{user}/folders/{folder}
   * Empty when the memo is ungrouped. Set on create or via UpdateMemo with
   * the "folder" mask path; set it to an empty string to make the memo
   * ungrouped. Only the folder owner may assign the folder, and the field is
   * only populated for the memo creator.
   *
   * @generated from field: string folder = 19;
   */
  folder: string;
};
...
export type ListMemosRequest = Message<"memos.api.v1.ListMemosRequest"> & {
   ...
   *   folder_uid (string; UID of the memo's folder, empty for ungrouped memos),
